...
|---|---|---|---|
| **The scanner had no positive control** — it only ever sent attack-shaped requests, so a server that refuses **everything**, including all legitimate use, passed every one of them. Measured 2026-08-19: a server answering HTTP 403 to every request scored **100%** | `RAN-LIVE` | `pytest tests/test_scanner_cli.py -k deny_all` | Found during an adversarial review by a local model (`qwen2.5`) whose other four findings were false positives; **the finding was reproduced here before being accepted**. A second-opinion model is not evidence — the reproduction is. Fixed by a `POSITIVE-CONTROL` check: with `--token` it sends one request that is *supposed* to succeed; without a token the score line carries the caveat |
| **The scanner was tied to KASA** — endpoint path and body field names were compiled into the source, so any other agent server returned `SKIP` on every check while the README claimed "audit any MCP server" | `RAN-LIVE` | `pytest tests/test_scanner_cli.py -k generic` | The narrowness only became visible **after** `SKIP` stopped being reported as `PASS`; before that a foreign server scored 60% and looked audited. Now driven by a JSON profile (`tools/scanner/profiles/`). The transport limit is unchanged and stated: HTTP+JSON only, no JSON-RPC envelope, no stdio, no SSE, no OAuth — for those the honest result is still `SKIP` |
...
|---|---|
| `pytest -q` | 384 passed, 1 xfailed (2026-08-19, py3.14.5). The xfail is an expected failure, not a pass — this is not "100% passing". Earlier runs of earlier trees: 323 (2026-08-05), 357 (before the scanner's three mock tests became fourteen fixture-driven ones) |
| `pytest tests/test_scanner_cli.py -q` | 31 passed (2026-08-19). Runs without `requirements.txt` if you add `--noconftest` and `PYTHONPATH=.` — the scanner is stdlib-only, which is why CI can run it on Linux while the full suite stays on Windows |
